...
class calculateNormalizedEntropy:
    ...
    def save_excel(self, param_entropy_list: list):
        df = pandas.DataFrame({
            'Shannon_Entropy': param_entropy_list
        })
        current_time = str(time.time())
        df.to_excel('./Data/shannon_result.xlsx', index=False)
        print('输出Excel完成')